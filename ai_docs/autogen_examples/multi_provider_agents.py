    ...
    async def create_client(
        ...
        if provider.lower() == "openai":
            return OpenAIChatCompletionClient(
                model=model_name, api_key=api_key or os.environ.get("OPENAI_API_KEY")
            )
        elif provider.lower() == "anthropic":
            return AnthropicChatCompletionClient(
                model=model_name, api_key=api_key or os.environ.get("ANTHROPIC_API_KEY")
            )
...
class AgentFactory:
    ...
    async def create_reporter_agent(model_client) -> AssistantAgent:
        ...
        return AssistantAgent(
            name="reporter", system_message=system_message, model_client=model_client
        )
...
async def analyze_code_with_multi_provider_agents(
    ...
    openai_client = await LLMClientFactory.create_client(
        "openai", "gpt-4o", openai_api_key
    )
    anthropic_client = await LLMClientFactory.create_client(
    ...
    try:
        ...
        language_detector = await AgentFactory.create_language_detector_agent(
            ollama_client
        )
        code_reviewer = await AgentFactory.create_code_reviewer_agent(openai_client)
        security_auditor = await AgentFactory.create_security_auditor_agent(
            anthropic_client
        )
...
async def main():
    ...
    results = await analyze_code_with_multi_provider_agents(
        sample_code, openai_api_key, anthropic_api_key
    )
