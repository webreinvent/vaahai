...
async def create_code_executor_agent(model_client) -> AssistantAgent:
    ...
    return AssistantAgent(
        name="code_executor", system_message=system_message, model_client=model_client
    )
...
async def create_code_reviewer_agent(model_client) -> AssistantAgent:
    ...
    return AssistantAgent(
        name="code_reviewer", system_message=system_message, model_client=model_client
    )
...
class DockerCodeExecutionWorkflow:
    ...
    async def execute_task(self, task_description: str) -> Dict[str, Any]:
        ...
        model_client = OpenAIChatCompletionClient(model="gpt-4o", api_key=self.api_key)
        ...
        try:
            ...
            user_proxy = UserProxyAgent(
                name="user_proxy", human_input_mode="NEVER"  # Don't ask for human input
            )
            ...
            for line in result.split("\n"):
                if "```python" in line:
                    ...
                    "```" in line and len(code_blocks) > 0 and len(code_blocks[-1]) > 0
                ):
                    code_blocks[-1] = "\n".join(code_blocks[-1])
            ...
            if not code_to_execute:
                return {"success": False, "error": "No executable code was generated"}
