...
def show_custom_help(ctx: typer.Context):
    ...
    * `VAAHAI_CONFIG_DIR` - Override default config directory location
    ...
    quick_reference.append(f"{command_name} --install-completion\n", style=STYLES["example"])
    ...
    console.print(Panel(Markdown(env_vars_md), title="Environment Variables", border_style=STYLES["table_border"], style=STYLES["section_title"]))
    console.print(Panel(quick_reference, title="Quick Reference", border_style=STYLES["table_border"], style=STYLES["section_title"]))
